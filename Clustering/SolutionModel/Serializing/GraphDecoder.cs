...
namespace Clustering.SolutionModel.Serializing
{
    public class ProjectTreeWithDependencies
    {
        ...
        public ProjectTreeWithDependencies(ISet<Node> nodes, ILookup<Node, Node> edges)
        {
            Nodes = nodes;
            Edges = edges;
        }
    ...
    public static class GraphDecoder
    {
        public static ProjectTreeWithDependencies Decode(string text)
        {
            ...
            return new ProjectTreeWithDependencies(nodes.ToSet(), deps);
        }

        private static ILookup<Node, Node> GetDependenciesFromFlatList(IEnumerable<FlatEntry> flats,
            Dictionary<string, Node> nodesByPath)
        {
            var edgeList =
        ...
        private static IEnumerable<Node> GetTreeFromFlatlist(IEnumerable<FlatEntry> descendants, int depth,
            IDictionary<string, Node> nodesByPath)
        {
